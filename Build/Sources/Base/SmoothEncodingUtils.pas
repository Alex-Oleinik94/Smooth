...
type
	TSEncoding = (
		SEncodingNull,
		SEncodingUTF8,
		SEncodingWindows1251,
		SEncodingCP866
		);
const
	SEncodingError   = SEncodingNull;
	SEncodingNone    = SEncodingNull;
	SEncodingWin1251 = SEncodingWindows1251;
	SEncodingW1251   = SEncodingWindows1251;
	SEncodingOEM866  = SEncodingCP866;
type
	TSEncodingDeterminer = class
			public
		class function DetermineEncoding(const VString : TSString) : TSEncoding;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
		class function IsWindows1251(const VString : TSString) : TSBoolean;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
		class function IsCP866(const VString : TSString) : TSBoolean;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
		class function IsUTF8(const VString : TSString) : TSBoolean;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
		end;

function SConvertString(const VString : TSString; const VEncoding : TSEncoding) : TSString;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}

procedure Windows1251ToUTF8(var Str: TSString);{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
procedure UTF8ToWindows1251(var Str: TSString);{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
procedure OEM866ToUTF8(var Str: TSString);{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
procedure UTF8ToOEM866(var Str: TSString);{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
procedure OEM866ToWindows1251(var Str: TSString);{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
procedure Windows1251ToOEM866(var Str: TSString);{$IFDEF SUPPORTINLINE}inline;{$ENDIF}

//Это функции для отладки WinAPI функций, связанных с символами, получиными в SmoothContextWinAPI как коды клавиш
function SWhatIsTheSimbol(const l : TSInt32; const Shift : TSBoolean = False ; const Caps : TSBoolean = False) : TSString;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
function SGetLanguage() : TSString;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
function SWhatIsTheSimbolRU(const l : TSInt32; const Shift : TSBoolean = False; const Caps : TSBoolean = False) : TSString;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
function SWhatIsTheSimbolEN(const l : TSInt32; const Shift : TSBoolean = False; const Caps : TSBoolean = False) : TSString;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}

function SConvertAnsiToASCII(const S : TSString) : TSString;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
...
function SConvertString(const VString : TSString; const VEncoding : TSEncoding) : TSString;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
var
	SourceFormat : TSEncoding = SEncodingNull;
begin
Result := VString;
SourceFormat := TSEncodingDeterminer.DetermineEncoding(VString);
if SourceFormat <> SEncodingNull then
	begin
	case SourceFormat of
	SEncodingUTF8 :
		case VEncoding of
		SEncodingWindows1251 : UTF8ToWindows1251(Result);
		SEncodingCP866       : UTF8ToOEM866     (Result);
		end;
	SEncodingWindows1251 :
		case VEncoding of
		SEncodingUTF8  : Windows1251ToUTF8  (Result);
		SEncodingCP866 : Windows1251ToOEM866(Result);
		end;
	SEncodingCP866 :
		case VEncoding of
		SEncodingUTF8        : OEM866ToUTF8       (Result);
		SEncodingWindows1251 : OEM866ToWindows1251(Result);
		end;
	end;
	end;
end;

{$INCLUDE SmoothEncodingCP866.inc}
{$INCLUDE SmoothEncodingWindows1251.inc}

class function TSEncodingDeterminer.IsUTF8(const VString : TSString) : TSBoolean;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
begin
Result := False;
end;

class function TSEncodingDeterminer.DetermineEncoding(const VString : TSString) : TSEncoding;{$IFDEF SUPPORTINLINE}inline;{$ENDIF}
begin
Result := SEncodingNull;
if (Result = SEncodingNull) and IsWindows1251(VString) then
	Result := SEncodingWindows1251;
if (Result = SEncodingNull) and IsCP866(VString) then
	Result := SEncodingCP866;
if (Result = SEncodingNull) and IsUTF8(VString) then
	Result := SEncodingUTF8;
end;

const
	SAnsiToASCII : packed array[TSChar] of TSChar = {Ansi - WINDOWS1251(CP1251); ASCII - CP866 }
...
var
	Language : TSString = '';
begin
Language := SGetLanguage();
if Language = 'EN' then
	begin
	Result := SWhatIsTheSimbolEN(l, Shift, Caps);
	end
else
	if Language='RU' then
		Result := SWhatIsTheSimbolRU(l, Shift, Caps)
	else
		Result := TSChar(l);
end;
